...
export function format(source, program, options = {}) {
  ...
  // Lines that are the middle of a piece of text belong to the writer, not
  // to the tidier. Touching them would change what the program says.
  const insideText = untouchableLines(source);
  ...
  for (let at = lines.length - 1; at >= 0; at--) {
    if (insideText.has(at + 1)) continue;
  ...
  for (let at = 0; at < lines.length; at++) {
    if (insideText.has(at + 1)) { out.push(lines[at]); blanks = 0; continue; }
...
// Every line that sits inside a piece of text spanning several lines. The
// first line is not one of them: that is where the sentence starts.
function untouchableLines(source) {
  const inside = new Set();
  let tokens;
  try { tokens = tokenize(source); } catch { return inside; }
  for (const token of tokens) {
    if (token.type !== 'text') continue;
    const overflow = String(token.value).split('\n').length - 1;
    for (let at = 1; at <= overflow; at++) inside.add(token.line + at);
  }
  return inside;
}
